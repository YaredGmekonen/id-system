...
import { wipeAllData, loadSampleTestPack } from '../db/seed';
import {
  AlertTriangle,
  CheckCircle2,
  Building2,
...
export default function SystemSettings() {
  ...
  // Reset Database (Clean Real Project Mode)
  const handleResetDatabase = () => {
    setConfirmModal({
      isOpen: true,
      title: 'Wipe All Demo & Project Records',
      message: 'Are you sure you want to completely clear all personnel records, folders, and templates? This will initialize a clean, real project workspace stored locally in your browser.',
      confirmText: 'Yes, Wipe All Data',
      onConfirm: async () => {
        setIsResetting(true);
        try {
          await wipeAllData();
          setSaveSuccessMsg('All database records cleared. Workspace is now a clean real project.');
          setTimeout(() => {
            window.location.reload();
          }, 800);
        } catch {
  ...
  const handleLoadSamplePack = () => {
    setConfirmModal({
      isOpen: true,
      title: 'Load Starter Sample Pack',
      message: 'Load 2 verified sample personnel records for testing card layouts?',
      confirmText: 'Load Samples',
      onConfirm: async () => {
        setIsResetting(true);
        try {
          await loadSampleTestPack();
          setSaveSuccessMsg('Sample test pack loaded.');
          setTimeout(() => {
            window.location.reload();
          }, 800);
        } catch {
          setSaveSuccessMsg('Failed to load sample pack.');
        } finally {
          setIsResetting(false);
        }
      },
          ...
          {activeTab === 'database' && (
            <div className="space-y-4">
              ...
              >
                ...
                {/* Workspace Data Management Zone */}
                <div className="pt-4 border-t space-y-3" style={{ borderColor: 'var(--border-primary)' }}>
                  <h3 className="font-bold text-xs text-red-400">
                    Workspace Reset & Real Project Initialization
                  </h3>
                  <p className="text-xs text-slate-400">
                    Wipe all mock/demo records, batch folders, and templates to start fresh with real local storage data.
                  </p>
                  <div className="flex flex-wrap items-center gap-3">
                    <button
                    ...
                    >
                      {isResetting ? 'Wiping...' : 'Wipe All Data & Start Fresh (Real Project Mode)'}
                    </button>
                    <button
                      onClick={handleLoadSamplePack}
                      disabled={isResetting}
                      className="px-4 py-2 border border-white/10 hover:bg-white/5 rounded-xl text-xs font-semibold text-slate-300 transition-colors cursor-pointer"
                      style={{ backgroundColor: 'var(--bg-elevated)' }}
                    >
                      Load Starter Sample Pack (2 Records)
                    </button>
                  </div>
                </div>
              </div>
